...
const ExclusiveToPetExpressCategoryCards = () => {
  const {
    handleDeleteCard,
    handleEditCard,
    handleMoveCard,
  ...
  const [categoryName, setCategoryName] = useState("dog-food");
  ...
  const filteredCards = useMemo(() => {
    ...
    return dataFromServerFiltered.filter(
      (item) => item.title.startsWith(filter) && item.price > 50
    );
  }, [query, dataFromServerFiltered]);

  if (!filteredCards.length) {
    return <Typography>Could not find any items under $15</Typography>;
  }
...
export default ExclusiveToPetExpressCategoryCards;
